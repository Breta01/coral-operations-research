...
class Grader(TaskGrader):
    ...
    def evaluate(self) -> ScoreBundle:
        ...
        if not _is_valid_score(score):
            return self.fail(f"Invalid non-finite/out-of-range score produced: {score!r}")
...
def _run_evaluation(
    ...
    with tempfile.TemporaryDirectory(prefix="drug_design_eval_") as tmpdir:
        molecules_path = os.path.join(tmpdir, "molecules.json")
        result = subprocess.run(
            [
                *python_cmd,
                "-c",
                _solution_runner_script(program_path, molecules_path),
            ],
            capture_output=True,
            text=True,
            timeout=timeout,
        ...
        if result.returncode != 0:
            raise RuntimeError(
                f"solution subprocess exited {result.returncode}.\n"
                f"stderr: {result.stderr.strip()[-2000:]}\n"
                f"stdout: {result.stdout.strip()[-500:]}"
            )
        if not os.path.exists(molecules_path):
            raise RuntimeError(
                "solution subprocess did not produce molecules.json.\n"
                f"stderr: {result.stderr.strip()[-1000:]}\n"
                f"stdout: {result.stdout.strip()[-500:]}"
            )
        try:
            payload = json.loads(open(molecules_path, encoding="utf-8").read())
        except json.JSONDecodeError as e:
            raise RuntimeError(f"solution produced invalid molecules JSON: {e}") from e
        if "error" in payload:
            return {"error": payload["error"]}

        molecules = payload.get("molecules")
        gen_time = float(payload.get("gen_time", 0.0))

    score_script = _scoring_script(molecules, scorer_dir, data_dir, top_k, gen_time)
    result = subprocess.run(
        [*python_cmd, "-c", score_script],
    ...
    try:
        ...
        raise RuntimeError(
            f"Scorer did not return a single JSON object.\n"
            f"stdout: {stdout[-500:]}\nstderr: {result.stderr.strip()[-500:]}"
        ) from e


def _solution_runner_script(program_path: str, output_path: str) -> str:
    return textwrap.dedent(f"""\
        ...
        def emit(payload):
            with _open(output_path, "w", encoding="utf-8") as f:
                f.write(_json_dumps(payload))

        try:
            path = {os.path.abspath(program_path)!r}
            ...
            if not hasattr(program, "run"):
                emit({{"error": "solution.py must define run()"}})
                sys.exit(0)
            ...
            if not isinstance(molecules, list) or len(molecules) == 0:
                emit({{"error": "run() must return a non-empty list of SMILES strings"}})
                sys.exit(0)
            emit({{"molecules": molecules, "gen_time": gen_time}})
        except Exception as e:
            emit({{"error": f"run() failed: {{type(e).__name__}}: {{e}}"}})
    """)
...
def _scoring_script(
    ...
    return textwrap.dedent(f"""\
        ...
        import numpy as _np; _np.float16 = _np.float32

        start = time.time()
        molecules = json.loads({json.dumps(molecules)!r})
        gen_time = {float(gen_time)!r}
        ...
        try:
            ...
            if is_available():
                ...
                kp_scores = [max(0.0, min(1.0, float(s))) if s is not None else 0.0 for s in kp_scores]
                has_minimol = True
        except Exception:
            pass
        ...
        try:
            ...
            if chemprop_avail():
                ...
                safety_scores = [max(0.0, min(1.0, float(s))) if s is not None else 0.5 for s in safety_scores]
                has_chemprop = True
        except Exception:
            pass
